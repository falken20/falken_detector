...
flake8 ./falken_detector/* --count --select=E9,F63,F7,F82 --show-source --statistics
# exit-zero treats all errors as warnings. The GitHub editor is 127 chars wide
echo "***** Linter: Checking Python syntax patterns *****"
flake8 ./falken_detector/* --count --exit-zero --max-complexity=10 --max-line-length=127 --statistics

# Unit test and coverage
echo "***** Unit Test *****"
coverage run -m pytest -v -s
echo "***** Coverage tests *****"
coverage report --omit="*/tests/*,*/venv/*" -m ./falken_detector/*.py 
